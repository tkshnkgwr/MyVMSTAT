...
AIがコードの変更、機能追加、リファクタリングなどを行う際は、必ず以下のドキュメントをセットで更新または作成すること。また、ドキュメントのファイル名は常に「大文字のスネークケース（`UPPER_SNAKE_CASE.md`）」で統一すること。
...
- **`docs/ARCHITECTURE.md`（システムアーキテクチャ設計書）の更新**:
  システム構造、モジュール間データフロー、または採用技術スタックに変更があった場合は、本設計書を更新すること。
- **`docs/INSTRUCTIONS.md`（コーディングスタイル規約）の更新**:
  コーディング基準、エラーハンドリング方針、AI出力フォーマット定義等に変更がある場合は、本規約を更新すること。
- **`docs/TODO.md`（ロードマップ・タスク管理）の更新**:
  機能の実装完了時や新規タスクの追加・バックログ起票時には、本ファイルを更新し、進捗状況を最新に保つこと。
- **`README.md` / `README.ja.md` の更新**:
  ビルド・実行手順、依存ライブラリの追加、設定手順に変更があった場合は、必ず英語・日本語双方のドキュメントに反映すること。また、READMEのヘッダー部には GitHub Actions のビルドステータスバッジ、プラットフォームバッジ、サポートするRustバージョンバッジ、およびライセンスバッジを必ず掲載し、双方の README 間の相互リンクを維持すること。
...
- **RustDoc (ドキュメントコメント) の自動更新**:
  ソースコードの変更、機能追加、またはリファクタリングを行う際は、必ず影響するモジュール（`//!`）、構造体、トレイト、関数、列挙型などの RustDoc コメント（`///`）を最新の仕様に合わせて修正・追記すること。特に新規追加した公開APIにはドキュメントコメントを必須とする。また、修正後は `cargo doc --no-deps` を実行してビルドが警告なく通ることを確認すること。
- **ドキュメントの多言語対応ポリシー**:
